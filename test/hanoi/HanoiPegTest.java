...
public class HanoiPegTest {
	...
	@Before
	public void setup() {
		impl = Configuration.getHanoiPegImplementation();
		if(impl == null)
	...
	public void testPeg() {
		...
		assertEquals("The top element we just put on top.", fiveRing, impl.getTopRing());
		
		HanoiRing fourRing = new HanoiRing(4);
		impl.addRing(fourRing);
		assertEquals("The top element should be fourRing", fourRing, impl.getTopRing());
		
		HanoiRing top = impl.remove();
		assertEquals("The removed element should be fourRing", fourRing, top);
		assertEquals("The top element should be fiveRing", fiveRing, impl.getTopRing());
